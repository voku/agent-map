...
final readonly class ParameterRenameCliApplication implements RenamePlanCliApplication
{
    ...
    public function capability(): RenamePlanCapability
    {
        return new RenamePlanCapability(
            ...
            planType: 'parameter_rename_plan',
            contractVersion: ParameterRenamePlan::CONTRACT_VERSION,
        );
    }
    ...
    {
        $command = $argv[1] ?? null;
        if ($command === 'parameter-rename-plan') {
            return true;
        }

        return $command === 'help' && ($argv[2] ?? null) === 'parameter-rename-plan';
    }
...
Parameter refactoring evidence:
  parameter-rename-plan Build a read-only, fail-closed method-parameter rename plan

Run `agent-map help parameter-rename-plan` for details.
TEXT;
    }
    ...
    public function run(array $argv): int
    {
        try {
            ...
            if (count($parsed['arguments']) !== 3) {
                throw new InvalidArgumentException('parameter-rename-plan requires exactly Class::method, originalParameter and replacementParameter.');
            }
            ...
            $plan = (new ParameterRenamePlanner())->plan(
                $map,
                $parsed['arguments'][0],
                $parsed['arguments'][1],
                $parsed['arguments'][2],
    ...
    private function render(ParameterRenamePlan $plan, string $format): string
    {
        return match ($format) {
    ...
    private function text(ParameterRenamePlan $plan): string
    {
        $lines = [
            sprintf('Parameter rename plan: %s', strtoupper($plan->status)),
            sprintf('Target: %s', $plan->targetId),
            sprintf('Parameter: %s -> %s (index %d)', $plan->originalName, $plan->replacementName, $plan->parameterIndex),
    ...
    {
        return <<<'TEXT'
Usage: agent-map parameter-rename-plan Class::method $old $new [--index PATH] [--format text|json|toon]
