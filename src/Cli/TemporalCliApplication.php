...
Temporal evolution:
  history diff   Compare two canonical map snapshots without treating line churn as architecture change
...
TEXT;
    ...
    private function diff(array $tokens): int
    {
        ...
        if ($parsed['arguments'] !== []) {
            throw new InvalidArgumentException('history diff accepts options only.');
        }
...
changes while deliberately ignoring line-number-only movement in relations. The command does not claim
why a change happened; rename, hotspot, and architecture-smell interpretations belong to later evidence.
TEXT;
    }
    ...
    private function render(array $payload, string $format, string $text): string
    {
        return match ($format) {
            ...
            'markdown' => "## Temporal structural diff\n\n```text\n" . rtrim($text) . "\n```\n",
            default => $text,
        };
    }
    ...
    private function json(array $payload): string
    {
        ...
        if (!is_string($json)) {
            throw new RuntimeException('Unable to encode temporal diff JSON.');
        }
