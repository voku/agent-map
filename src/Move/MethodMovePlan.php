...
/**
 * Versioned, read-only plan for relocating one method to an explicitly chosen
 * destination class.
 *
 * agent-map never decides that a method should move and never proposes a
 * destination; both identities are supplied by the caller. What the plan adds is
 * the mechanical evidence: where the declaration is, what owner state it still
 * observes, where it can be inserted, and which call sites are semantically
 * resolved enough to re-point.
 */
final readonly class MethodMovePlan implements GovernedPlan
{
    public const PLAN_TYPE = 'method_move_plan';
