...
final readonly class ClassRenameScopeGuard
{
    ...
    public function assertReplacementAvailable(
        ...
        $source = $this->locator->sourceFor($path);
        $ast = $this->locator->astFor($path);
        if (!$this->requiresReplacementShortName($ast, $source, $targetFqn, $targetShort)) {
            return;
        }
    ...
    private function requiresReplacementShortName(array $ast, string $source, string $targetFqn, string $targetShort): bool
    {
        foreach ($this->nodes($ast) as $node) {
            if ($node instanceof Use_) {
                foreach ($node->uses as $use) {
                    if ($this->isClassUse($use, $node->type)
                        && $this->importUsesReplacementShortName($use, $use->name->toString(), $targetFqn)) {
                        return true;
                    }
                }
                continue;
            ...
            if ($node instanceof GroupUse) {
                ...
                foreach ($node->uses as $use) {
                    if ($this->isClassUse($use, $node->type)
                        && $this->importUsesReplacementShortName($use, $prefix . '\\' . $use->name->toString(), $targetFqn)) {
                        return true;
                    }
                }
                continue;
    ...
    private function importUsesReplacementShortName(UseItem $use, string $importFqn, string $targetFqn): bool
    {
        return $use->alias === null && strcasecmp(ltrim($importFqn, '\\'), $targetFqn) === 0;
    }
    ...
    private function sourceToken(string $source, Name $name): string
    {
        ...
        if ($start < 0 || $end < $start) {
            throw new RuntimeException('Parser did not expose byte positions for class rename scope evidence.');
        }
