...
final readonly class ClassRenamePlanner
{
    ...
    private const NOT_OBSERVABLE = [
        ...
        'PHPDoc class references and exact class-string literals are review evidence rather than automatic edits in the initial contract.',
    ];
    ...
    public function plan(AgentMapIndex $map, string $target, string $replacement): ClassRenamePlan
    {
        ...
        foreach ($map->files as $candidateFile) {
            foreach ($candidateFile->symbols as $candidate) {
                if (!in_array($candidate->kind, ['class', 'interface', 'trait', 'enum'], true) || $candidate->id() === $symbol->id()) {
                    continue;
                }
        ...
        if ($staleEvidence !== [] || $blockers !== []) {
            return $this->result($map, $symbol, $replacementFqn, [], [], $blindSpots, $staleEvidence, $blockers);
        }
