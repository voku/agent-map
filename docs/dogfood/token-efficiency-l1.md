...
Run a controlled, falsifiable case study using contract anchor
`2124c244365726469197c2532a9c4bc1f878cc31` plus the frozen `portable-ascii-135` fixture to test whether Map-first navigation reduces
...
Run only this already-solved, pre-registered replay rather than scaling an invalid harness:
...
Do not run `simple-php-code-parser-101`, `portable-ascii-62`, repeated trials, or additional hosts in
this slice. They are gated on one valid isolated A/B pair.
...
- Controller repository: `/workspace/agent-map`; commit `2124c244365726469197c2532a9c4bc1f878cc31`
  must be an ancestor. Record the exact execution commit and fixture SHA-256 in the manifest, and abort
  as invalid if tracked files are dirty before orchestration.
- The task definition, executable validation, and pre-registered ground truth are in
  `tools/dogfood/replays/portable-ascii-135.json`.
  Copy only the fixture's `experiment_task` object into the arm prompt. Keep `verified` hidden from the coding
  agent and use it only for independent post-run grading.
- The fixture freezes the already-decided coding intent, historical patch digest, required/allowed files and ranges, acceptance
  assertions, and executable regression, full-test, and static-analysis commands. Keep its `verified`,
  `grading_note`, `fix_commit`, and `validation.diff_constraints` fields hidden from coding agents.
- Available Map surfaces are `query`, `file`, `related`, `scope`, `context`, `callers`, `callees`,
  ...
  relevant surface may be used. No typed change plan is applicable to this task, so plan
  metrics are `NOT_APPLICABLE`, not zero.
- Freeze one Map backend before the pair starts. Its requested identity must match the map's recorded
  `analysis_fingerprint`; structural operation is explicit, never a fallback after PHPStan fails.
- Create one detached clean worktree per arm from the frozen upstream commit. Run each arm
  in a new coding-model conversation with no shared transcript, cache, snippets, path hints, patches,
  summaries, or controller memory. This slice is one trial and must be labelled a non-statistical case study.
...
- **Arm B — Map-first:** for represented PHP facts, require the narrowest progression: exact known
  ...
  orientation. For literals, templates, prose, generated artifacts, an explicit Map blind spot, or
  post-selection source verification, permit `rg` and bounded reads.
...
1. Preflight the controller with:
   ...
   ```bash
   git -C /workspace/agent-map merge-base --is-ancestor 2124c244365726469197c2532a9c4bc1f878cc31 HEAD
   test -z "$(git -C /workspace/agent-map status --porcelain --untracked-files=no)"
   composer --working-dir=/workspace/agent-map ci
   jq -e '
     .id == "portable-ascii-135" and
     .base_commit == "88f94f89fe03bed03eb8fbcfb84178a8a5eb1d5b" and
     (.experiment_task.text | length > 0) and
     (.validation.setup_commands | length > 0) and
     (.validation.regression_command | length > 0) and
     (.validation.test_command | length > 0) and
     (.validation.static_analysis_command | length > 0) and
     (.validation.acceptance_assertions | length > 0) and
     (.validation.diff_constraints.required_files | length > 0) and
     (.validation.diff_constraints.allowed_files | length > 0) and
     (.validation.diff_constraints.required_ranges | length > 0) and
     (.validation.diff_constraints.historical_patch_sha256 | test("^[0-9a-f]{64}$"))
   ' /workspace/agent-map/tools/dogfood/replays/portable-ascii-135.json
   ```

   The final command proves this task's validation contract is complete before either arm. Record any
   failure; do not improvise acceptance criteria after seeing an arm.
