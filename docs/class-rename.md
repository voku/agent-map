...
- `blocked`: no mutation plan is published because stale evidence exists, the target is ambiguous, the replacement class/file already exists, a namespace move was requested, or exact token evidence cannot be produced.

## Exact edits

Machine output is `class_rename_plan` contract version `1.0`, with map digest, effective backend, and analysis fingerprint under `provenance`; stale source evidence is machine-distinct from semantic blockers.
