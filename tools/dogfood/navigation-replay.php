 ...
 * reading models are reported (whole-file and a 40-line window around the match) so the comparison
 * cannot be won by charging the baseline for reads a careful agent would not make.
 *
 * Usage:
...
function observationEnvelope(array $replay, AgentMapIndex $index, SearchIndexStore $store, string $requestedBackend): array
{
    ...
    return [
        ...
        'phpstan_available' => is_file(__DIR__ . '/../../vendor/bin/phpstan'),
        'not_observable' => [
...
if (isset($arguments['json'])) {
    file_put_contents($arguments['json'], $json . "\n");
}
echo $json . "\n";
